<template>
    <v-card outlined>
        <v-card-title>
            VolunteerInfo # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Date label="Birth" v-model="item.birth" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/volunteerInfos/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
