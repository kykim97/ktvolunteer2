<template>
    <v-card outlined>
        <v-card-title>
            AssociationInfo # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>

        <v-card-text>
            <div>
                <Number label="AssociationId" v-model="item.associationId" :editMode="editMode" @change="change" />
            </div>
            ...
            <div>
                <Number label="BizNumber" v-model="item.bizNumber" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/associationInfos/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
