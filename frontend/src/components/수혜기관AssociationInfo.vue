<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            수혜기관정보 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            수혜기관정보
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <String label="위치" v-model="value.place" :editMode="editMode" :inputUI="''"/>
            <String label="일정" v-model="value.schedule" :editMode="editMode" :inputUI="''"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/associationInfos'), this.value)
                        } else {
